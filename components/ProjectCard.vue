<template>
    <button ref="btn" class="flex flex-col space-y-4 m-8 p-4 w-80 h-full cursor-pointer bg-white dark:bg-slate-700 rounded-md shadow-md border hover:border-slate-300 hover:dark:border-slate-500 transition-all" @click="openModal"
        :class="modalOpen? 'border-primary dark:border-primary' : 'border-slate-200 dark:border-slate-600'">
...
watch(modalOpen, (val, old) => {
    ...
    if (!modalOpen.value && router.currentRoute.value.query.project === props.project.name) {
        router.push({ query: { project: undefined } });
    }
});
