<template>
    <div class="flex flex-col grow min-h-full w-full">
        ...
        <icon-header
            ...
            :show-icon="false"
        />
        <div class="show-up flex flex-col grow m-4 border-2 border-slate-400 rounded-lg overflow-hidden">
            <div class="flex grow h-fit border-b-2 bg-slate-100 border-slate-200 p-2 space-x-2">
                ...
                <div class="show-left flex grow m-auto min-w-0">
                    <p class="h-fit min-w-0 w-full text-slate-600 text-xl font-semibold whitespace-nowrap text-ellipsis overflow-hidden">
                        {{ element?.name }}
                    </p>
                </div>
            </div>
            <div class="min-h-0 h-full bg-white overflow-auto">
                <div class="h-fit w-full divide-y-2">
                    <div
                        v-for="el in element?.elements"
                        :key="el"
                        class="flex grow px-2 py-4 hover:bg-slate-100 cursor-pointer"
                        :class="animationClass"
                        @click="selectElement(el)"
                    >
                        <div class="flex w-min my-auto mr-4">
                        ...
                        <div class="flex grow m-auto">
                            <p class="h-fit text-slate-700 text-md">
                                {{ el?.name }}
                            </p>
                        </div>
                    </div>
