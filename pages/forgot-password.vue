        ...
        <div class="flex flex-col grow justify-evenly items-center">
            ...
            <div class="flex h-fit w-full justify-center items-center px-4 overflow-hidden">
                <div class="flex h-fit min-w-full justify-center items-center">
                    <UForm :schema="formSchema" :state="formState"
                        class="show-up p-4 space-y-4 w-full max-w-[20em]" @submit="onformSubmit">
                        <UFormGroup :label="$t('login.email')" name="email">
                            <UInput v-model="formState.email" autocomplete="email" />
                        </UFormGroup>

                        <UAlert v-show="formError" @close="formError = null" :title="formError ?? ''"
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <UButton :loading="formButtonLoading" :disabled="formButtonDisabled" type="submit" icon="i-heroicons-chevron-right"
                                trailing>
                                {{ $t('verb.continue') }}
                            </UButton>
                        </div>
...
async function onformSubmit(event: FormSubmitEvent<formSchema>) {
    ...
    const res = await API.Request(ROUTES.PASSWORD.SENDRESETMAIL(event.data.email));
    if (res.error) {
        console.error('Error sending mail in:', res.message);
